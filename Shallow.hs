...
plot width height region = 
    let rows = [ [if region (fromIntegral x, fromIntegral y) then '*' else '.' | x <- [-width..width]] | y <- reverse [-height..height]]
    in intercalate "\n" rows
...
{-
    *** TODO ***
    ...
    5.0
-}
promoteUnary :: (a -> b) -> (Point -> a) -> (Point -> b)
promoteUnary f g = f . g

promoteBinary :: (a -> b -> c) -> (Point -> a) -> (Point -> b) -> (Point -> c)
promoteBinary f g h point = f (g point) (h point)
...
circles n
    | n <= 0    = const False
    | otherwise = Shallow.union (circle 2)
...
infiniteCircles = Shallow.union (circle 2)
                        (applyTransformation (translation 6 0)
                                             infiniteCircles)
...
bfs start expand = bfs' S.empty [(start, 0)]
  where
    bfs' _ [] = []
    bfs' visited ((state, dist):queue)
      | S.member state visited = bfs' visited queue
      | otherwise = (state, dist) : bfs' (S.insert state visited) (queue ++ neighbors)
      where
        neighbors = [(n, dist + 1) | n <- expand state, not (S.member n visited)]
...
regionAvoidingBfs start region = bfs start expand
    where
        expand (x, y) = filter (not . region) [(x,y-1),(x,y+1),(x-1,y),(x+1,y)]
