...
instance Show TransformationAST where
  ...
  show (T (Combine transformations)) = "[" ++ intercalate "," (map show transformations) ++ "]"
...
instance Show RegionAST where
  show r = foldRegionAST combiner r 0
    where
      combiner :: RegionShape (Int -> String) -> Int -> String
      combiner shape level = case shape of
        FromPoints points -> replicate (2 * level) ' ' ++ "FromPoints " ++ show points
        Rectangle l w -> replicate (2 * level) ' ' ++ "Rectangle " ++ show l ++ " " ++ show w
        Circle r -> replicate (2 * level) ' ' ++ "Circle " ++ show r
        Complement r -> replicate (2 * level) ' ' ++ "~\n" ++ r (level + 1)
        Union r1 r2 -> replicate (2 * level) ' ' ++ "+\n" ++ r1 (level + 1) ++ "\n" ++ r2 (level + 1)
        Intersection r1 r2 -> replicate (2 * level) ' ' ++ "*\n" ++ r1 (level + 1) ++ "\n" ++ r2 (level + 1)
        Transform t r -> replicate (2 * level) ' ' ++ show t ++ "\n" ++ r (level + 1)
...
instance Functor TransformationShape where
  fmap _ (Translation dx dy) = Translation dx dy
...
instance Functor RegionShape where
  fmap _ (Circle r) = Circle r
  fmap f (Union a b) = Union (f a) (f b)
  fmap _ (FromPoints points) = FromPoints points
  fmap _ (Rectangle l w) = Rectangle l w
  fmap f (Complement r) = Complement (f r)
  fmap f (Intersection a b) = Intersection (f a) (f b)
  fmap f (Transform t r) = Transform t (f r)
...
toTransformation = foldTransformationAST combiner
  where
    ...
    combiner (Translation dx dy) = S.translation dx dy
    combiner (Scaling s) = S.scaling s
    combiner (Combine transformations) = foldl (flip (.)) id transformations
...
-}
countTransformations :: TransformationAST -> Int
countTransformations (T shape) = case shape of
  Translation _ _ -> 1
  Scaling _ -> 1
  Combine transformations -> sum (map countTransformations transformations)
...
basicEntityCount = foldRegionAST combiner
  ...
  combiner (Complement (regions, transformations)) = (regions, transformations + 1)
  combiner (Union (regions1, transformations1) (regions2, transformations2)) = 
    (regions1 + regions2, transformations1 + transformations2)
  combiner (Intersection (regions1, transformations1) (regions2, transformations2)) = 
    (regions1 + regions2, transformations1 + transformations2)
  combiner (Transform transformations (regions, _)) = 
    (regions, countTransformations transformations)
