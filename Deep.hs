...
decomposeTransformation transformation = 
    let stack = [transformation]
    in decomposeStack stack []

decomposeStack :: [TransformationAST] -> [TransformationAST] -> [TransformationAST]
decomposeStack [] result = result
decomposeStack (top:rest) result =
    case top of
        Combine transformations -> decomposeStack (transformations ++ rest) result
        _ -> decomposeStack rest (result ++ [top])
...
fuseTransformations transformations =
    let fuse acc t =
            case (acc, t) of
                ((Translation x1 y1 : ts), Translation x2 y2) ->
                    Translation (x1 + x2) (y1 + y2) : ts
                ((Scaling s1 : ts), Scaling s2) ->
                    Scaling (s1 * s2) : ts
                (_, t') -> t' : acc
    in reverse (foldl' fuse [] transformations)
...
optimizeTransformations (FromPoints points) = undefined
